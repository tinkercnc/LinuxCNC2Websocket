...
var initWebSocket = function(host) { 
    ...
    ws.onmessage = function (evt) {
		...
		$("#canvas").clearCanvas({x: 5, y: 20, width: 1000, height: 48});
		//var tYsize=$("#canvas").measureText('Y');
		$("#canvas").clearCanvas({x: 5, y: 70, width: 1000, height: 48});
		//var tZsize=$("#canvas").measureText('Z');
		$("#canvas").clearCanvas({x: 5, y: 120, width: 1000, height: 48});
		$("#canvas").clearCanvas({x: 5, y: 170, width: 1000, height: 48});
		
		bigText("X: ", position[0]+' mm', 5, 20);
		bigText("Y: ", position[1]+' mm', 5, 70);
		bigText("Z: ", position[2]+' mm', 5, 120);
		bigText("Vel: ", position[3]+' mm/min', 5, 170);
...
var drawAll = function() {
    ...
    bigText("X: ", 0+' mm', 5, 20);
	bigText("Y: ", 0+' mm', 5, 70);
	bigText("Z: ", 0+' mm', 5, 120);
	bigText("Vel: ", 0+' mm/min', 5, 170);
    //drawX(0);
    //drawY(0);
    //drawZ(0);
};
